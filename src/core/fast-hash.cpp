...
FastHash::FastHash(const std::string &aof_path, AOFSyncPolicy policy)
    : aof_logger_(aof_path, policy), is_aof_loading_(false) {
...
bool FastHash::set(const std::string &key, const std::string &value,
  ...
  if (ttl_seconds.has_value()) {
    ...
    if (!is_aof_loading_)
      aof_logger_.log("SETEX " + key + " " +
                      std::to_string(ttl_seconds.value()) + " " + value);
  } else {
...
bool FastHash::load(const std::string &filepath) {
  ...
  for (auto it = data.begin(); it != data.end(); ++it) {
    ...
    if (!entry.contains("value") || !entry["value"].is_string()) {
      std::cerr << "[ERROR] Invalid entry for key '" << key
                << "', missing value\n";
      continue;
    }
...
void FastHash::replayAOF(const std::string &filepath) {
  ...
  std::cout << "[INFO] AOF replayed " << count << " commands\n";
}
